    ...
    <div className="container mx-auto py-12 md:py-24 px-4 md:px-6">
        ...
        >
          {isLoading ? (
            ...
            filteredArticles.map((article) => (
              <motion.div layout key={article.id} animate={{ opacity: 1 }} initial={{ opacity: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }}>
      ...
      {!isLoading && filteredArticles.length === 0 && (
          <div className="mt-16 text-center text-muted-foreground col-span-full">
              <p>{searchTerm ? "Aucun article ne correspond à votre recherche." : "Aucun article publié pour le moment. Revenez bientôt !"}</p>
          </div>
      )}
    </div>
  );
